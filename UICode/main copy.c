...
{   
    ...
    while (1) {
        ...
        case 2: // scan and log
            ...
            // int *pidResps = malloc(numToLog * sizeof(int)); // arr to hold resp from each query
            // int index = 0;
            // if (!pidResps) {
            //     printf("Failed to allocate memory for PID responses.\n");
            //     free(chosenPIDS);
            //     free(BlackImage);
            //     DEV_Module_Exit();
            //     return -1; // exit with error
            ...
            while (1) {
                ...
                if (currentPID < numToLog) {
                    ...
                    if (entry.supported) {
                        ...
                        strncat(strBuffer, valueBuffer, 256 - strlen(strBuffer) - 1);
                        if (currentPID == 0) {
                            mainVal = val;
                            draw_Number(BlackImage, mainVal);
                        }
                    }
                    currentPID++;
                }
                if (elapsed_time >= 1000000) { // log every second
                    ...
                    if (currentPID < numToLog) {
                        for (int i = currentPID; i < numToLog; i++) {
                            strncat(strBuffer, ",", 256 - strlen(strBuffer) - 1); // fill with empty values
                        }
                    }
                    snprintf(record, sizeof(record), "%d", time);
                    strncat(record, strBuffer, 256 - strlen(strBuffer) - 1);
                    strncat(record, "\n", 256 - strlen(record) - 1);
                    printf("%s", record);
                    // selectSD();
                    sdUsed = true;
